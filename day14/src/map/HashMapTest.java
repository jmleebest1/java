...
public class HashMapTest {
   public static void main(String[] args) {
      ...
      //학생에 대한 정보이므로 키 값은 Student타입으로 하고 3개의 정보를 다루므로 value는 ArrayList로 한다. 과목이 추가되어도 추가 가능하다
      //학생 객체 선언
      ...
      studentMap.put(std1, std1Scores);//첫번째 학생에 첫번째 점수를 넣어주고
      studentMap.put(std2, std2Scores);//두번째 학생에 두번째 점수를 넣어준다.
      ...
      System.out.println(studentMap.containsKey(std3));//containsKey,containsValue : 키와 밸류가 있는지 확인하는 메소드
      //studentMap에 std3의 정보가 있는지 확인
...
//      //학생 1번의 점수를 수정한다.
//      studentMap.put(std3, std3Scores); //map은 수정 메서드가 set()이 아니라 put()으로 한다.
...
//      }
      ...
      for(ArrayList<Integer> scoreList : scores) {//각 학생의 점수들을 scoreList에 담아준다.
         ...
         //전체 평균 구하기
         avg =(double)total /  (scoreList.size() * studentMap.keySet().size());  ////////////////////이부분 이해 안감;;;
