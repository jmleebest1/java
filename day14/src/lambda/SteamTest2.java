...
//	   IntStream.range(1, 101).forEach(v -> datas.add(v));
...
//      System.out.println(datas.stream().collect(Collectors.joining(" ")));
      ...
      //전체 대문자로 변경 후 출력
//      datas.stream().map(color -> color.toUpperCase())
//      .forEach(color -> System.out.println(color));//내가 한거
//      	datas.stream().map(c -> c.toUpperCase()).forEach(c -> System.out.println(c));
