...
public class StudentField {
	...
	public boolean update(Student std) {//std는 수정이 완료된 정보가 들어가는거다
		for (int i = 0; i < students.size(); i++) {
			if(students.get(i).getNum() == std.getNum()) {
				//데이터베이스에 있는 것과 사용자가 입력한 번호를 비교
				students.set(i, std);//i번째 방에 있는걸 std로 바꿔준다
				return true;
			}
		}
		return false;
	...
	public ArrayList<Student> selectNames(String name) {//ArrayList에 담아서 리턴한다
