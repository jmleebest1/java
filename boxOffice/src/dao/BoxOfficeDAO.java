...
public class BoxOfficeDAO {
   ...
   public void append(BoxOfficeVO film) throws IOException{
      BufferedReader br = DBConnecter.getReader();
      String line = null;
   ...
   public void insert(BoxOfficeVO film) throws IOException{
      //순위 검사
   ...
   public void update(BoxOfficeVO film) throws IOException{
      ...
      while((line = br.readLine()) != null) {
         ...
         if(Integer.parseInt(line.split("\t")[0]) == film.getRanking()) {
            ...
            temp += film.toString() + "\n";
            //기존 정보 대신 넣은 것이기 때문에 아래의 기존정보를 담는 코드를 실행하지 않기 위해서 continue를 사용한
            continue;
         }
   ...
   //검색 결과가 한 개가 아니라 여러개이므로 ArrayList에 담아서 리턴한다.
